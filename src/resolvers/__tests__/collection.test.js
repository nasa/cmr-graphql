...
describe('Collection', () => {
  ...
  describe('Query', () => {
    ...
    describe('collections', () => {
      ...
      test('search by collectionProgress parameter', async () => {
        nock(/example-cmr/)
          .defaultReplyHeaders({
            'CMR-Hits': 1,
            'CMR-Took': 7,
        ...
        const response = await server.executeOperation({
          variables: {},
          query: `{
            collections(params: { collectionProgress: "COMPLETE" }) {
              items {
                conceptId
                shortName
                collectionProgress
    ...
    describe('collection', () => {
      ...
      describe('with revisionId parameter', () => {
        test('returns a specific revision', async () => {
          nock(/example-cmr/)
            .defaultReplyHeaders({
              'CMR-Hits': 3,
              'CMR-Took': 7,
            ...
            .reply(200, {
              ...
              items: [{
                ...
                umm: {
                  EntryTitle: 'Latest Version',
                  Abstract: 'This is the latest version'
                }
              }, {
                meta: {
                  'concept-id': 'C100000-EDSC',
                ...
                umm: {
                  EntryTitle: 'Middle Version',
                  Abstract: 'This is the middle version'
                }
              }, {
                meta: {
                  'concept-id': 'C100000-EDSC',
                ...
                umm: {
                  EntryTitle: 'Original Version',
                  Abstract: 'This is the original version'
                }
              }]
            })
          ...
          const response = await server.executeOperation({
            ...
            query: `query Collection($params: CollectionInput) {
        ...
        test('throws error when JSON-only fields are requested', async () => {
          const response = await server.executeOperation({
            variables: {
              params: {
                conceptId: 'C100000-EDSC',
          ...
          expect(errors[0].message).toContain('The following fields are not available when querying a specific revision: browseFlag')
        })

        test('throws error when multiple JSON-only fields are requested', async () => {
          const response = await server.executeOperation({
            variables: {
              params: {
                conceptId: 'C100000-EDSC',
          ...
          expect(errors).toBeDefined()
          expect(errors[0].message).toContain('The following fields are not available when querying a specific revision')
          expect(errors[0].message).toContain('browseFlag')
          expect(errors[0].message).toContain('hasGranules')
