...
describe('AclConcept', () => {
  ...
  describe('parse', () => {
    test('successfully parses response with jsonKeys', async () => {
      ...
      const jsonResponse = { data: { items: [{ concept_id: '123', key1: 'value1', key2: 'value2' }] } }
      jest.spyOn(aclConcept, 'getResponse').mockResolvedValue([jsonResponse]);

      await aclConcept.parse({ jsonKeys: ['key1', 'key2'] })

      expect(aclConcept.getItems()).toEqual({ '123': { key1: undefined, key2: undefined } })
    })
  ...
  describe('fetchAcl', () => {
    test('fetchAcl calls aclQuery with correct parameters when jsonKeys are present', () => {
      ...
      aclConcept.fetch({ key1: 'value1', key2: 'value2' });

      expect(aclQuerySpy).toHaveBeenCalledWith({"key1": "value1", "key2": "value2"}, ["key1", "key2"], {"Header-Key": "header-value"})
    })
